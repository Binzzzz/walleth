...
class FileBackedTokenProvider(context: Context, val networkDefinitionProvider: NetworkDefinitionProvider) : TokenProvider {
    ...
    override fun getAllTokens() = holder.tokenDefinitions[networkDefinitionProvider.value!!.getNetworkName()]!!.toList()

    override fun addToken(tokenDescriptor: TokenDescriptor) {
        holder.tokenDefinitions[networkDefinitionProvider.value!!.getNetworkName()]!!.add(tokenDescriptor)
        save()
    }
