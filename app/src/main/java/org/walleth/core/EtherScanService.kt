...
class EtherScanService : LifecycleService() {
    ...
    fun queryEtherscanForBalance(addressHex: String) {
        ...
        if (blockNum != null) {
            if (currentToken == ETH_TOKEN) {
                val balanceString = getEtherscanResult("module=account&action=balance&address=$addressHex&tag=latest")?.getString("result")

                if (balanceString != null) {
                    balanceProvider.setBalance(Address(addressHex), blockNum, BigInteger(balanceString), ETH_TOKEN)
                }
            } else {
                val balanceString = getEtherscanResult("module=account&action=tokenbalance&contractaddress=${currentToken.address}&address=$addressHex&tag=latest")?.getString("result")

                if (balanceString != null) {
                    balanceProvider.setBalance(Address(addressHex), blockNum, BigInteger(balanceString), currentToken)
                }
            }
        }
    }
    ...
    fun getEtherscanResult(requestString: String): JSONObject? {
        ...
        return try {
            newCall.execute().body().use { it?.string() }.let { JSONObject(it) }
